...
func TestRegression(t *testing.T) {
	...
	t.Run("jobs-stub", func(t *testing.T) {
		...
		// command returns 202 (queued) — jobs are a no-op stub
		if w := do(r, "POST", "/api/jobs/thumbnail-generation", token, mustJSON(t, gin.H{}), "application/json"); w.Code != 200 && w.Code != 202 {
			t.Errorf("jobs command -> %d", w.Code)
		}
	})
