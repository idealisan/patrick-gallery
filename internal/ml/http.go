...
func (b *HTTPBackend) Infer(req Request) (Result, error) {
	...
	if err != nil { return Result{}, fmt.Errorf("ml http encode: %w", err) }
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, b.config.Endpoint, bytes.NewReader(body))
	if err != nil { return Result{}, fmt.Errorf("ml http request: %w", err) }
	httpReq.Header.Set("Content-Type", "application/json")
	if b.config.Token != "" { httpReq.Header.Set("Authorization", "Bearer "+b.config.Token) }
	resp, err := b.client.Do(httpReq)
	if err != nil { return Result{}, fmt.Errorf("ml http transport: %w", err) }
	defer resp.Body.Close()
	data, err := io.ReadAll(io.LimitReader(resp.Body, 32<<20))
	if err != nil { return Result{}, fmt.Errorf("ml http read: %w", err) }
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return Result{}, fmt.Errorf("ml http status %d: %s", resp.StatusCode, strings.TrimSpace(string(data)))
	}
	var result Result
	if err := json.Unmarshal(data, &result); err != nil { return Result{}, fmt.Errorf("ml http decode: %w", err) }
	return result, nil
}
