...
|---|---|---|---|---|---|
| 1 | `internal/app/compat_v3.go:37` `handleServerStorage` | `GET /server/storage` | Constant `0 B` / `0` for disk fields | Implement real disk usage of library dir, or honest `501` | ✅ trivial |
...
| 3 | `internal/app/compat_v3.go:78` `handleSyncAck` | `GET/POST/DELETE /sync/ack` | Returns `{ack, type:"AssetV1"}`, no state change | Implement real ack persistence (record consumed deltas) or honest `501` | ✅ |
| 4 | `internal/app/compat_v3.go:86` `handleSyncStream` | `GET /sync/stream` | No-op `[]any{}` delta stream | Implement real delta stream, or honest `501` (verify client still syncs via websocket) | ✅ / ⚠️ needs real-app test |
...
| 9 | `internal/app/compat_v3.go:114` `handleSearchCities` | `GET /search/cities` | `[]any{}` | Implement city search from `geo/cities.tsv` | ✅ (geo data present) |
| 10 | `internal/app/compat_v3.go:118` `handleSearchPlaces` | `GET /search/places` | Empty `places/recentPlaces/allPlaces` | Implement from `geo/*` data | ✅ (geo data present) |
| 11 | `internal/app/compat_v3.go:124` `handleDownloadInfo` | `POST /download/info` | `{size:0}` constant | Compute real archive size on demand, or honest `501` | ✅ |
| 12 | `internal/app/compat_v3.go:128` `handleDuplicatesResolve` | `POST /duplicates/resolve` | `200` no-op | Implement (mark resolved / merge) or honest `501` | ✅ |
| 13 | `internal/app/compat_v3.go:132` `handleDuplicatesUpdate` | `PUT/DELETE /duplicates/:id`, `DELETE /duplicates` | `200` no-op | Implement or honest `501` | ✅ |
